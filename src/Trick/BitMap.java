...
public class BitMap {
    /**
     * 位图提供的最大长度,判断一个数是否在40亿个整数中？
     * 比如unsigned int的最大值为4294967295, 则需要的length为4294967296
     * <p>
     * Ref: https://blog.csdn.net/v123411739/article/details/86652806
     */
    private long length;
    ...
    /**
     * length为1 - 32: 需要1个桶（2^5）
     * length为33 - 64: 需要2个桶
     * ...
     * 以此类推
     * length = 2^32: 需要 2^27个桶
     *
     * @param length
     */
    public BitMap(long length) {
        this.length = length;
        // 根据长度算出，所需位图桶个数
        // ((length & 31) > 0 ? 1 : 0)这个是当，length<32时，保证至少一个桶
